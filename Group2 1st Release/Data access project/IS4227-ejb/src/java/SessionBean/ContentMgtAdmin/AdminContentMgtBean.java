...
//            }
//        }
        Query q = em.createQuery("SELECT m FROM ItemEntity m WHERE m.winery.id=?1");
        q.setParameter(1, id);       
        if(!q.getResultList().isEmpty()){
            System.out.println("item associates winery");
            return false;
        }
        em.remove(winery);
        return true;
...
//        System.out.println("before remove");
        Query q = em.createQuery("SELECT m FROM ItemEntity m WHERE m.region.id=?1");
        q.setParameter(1, id);       
        if(!q.getResultList().isEmpty()){
            System.out.println("item associates region");
            return false;
        }
        em.remove(region);
        return true;
...
//            }
//        }
        Query q = em.createQuery("SELECT m FROM ItemEntity m WHERE m.category.id=?1");
        q.setParameter(1, id);       
        if(!q.getResultList().isEmpty()){
            System.out.println("item associates category");
            return false;
        }
        em.remove(category);
        return true;
